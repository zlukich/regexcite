...
devtools::install_github("zlukich/regexcite")
```
...
Use `new_function_name()` when the input is known to be a single string.
For safety, it will error if its input has length greater than one.

`new_function_name()` is built on `stringr::str_split()`, so you can use its `n` argument and stringr's general interface for describing the `pattern` to be matched.
